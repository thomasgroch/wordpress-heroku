...
class WC_Correios_Connect {
	...
	public static function fix_currency_format( $value ) {
		// Se ja possui formatacao com ponto, apenas troca parar virgula
		if ( substr( $value, -3, 1 ) == '.') {
			$value = str_replace( ',', '.', $value );
			return $value;
	...
	private function calcEnvioFacil( $zip_destination, $zip_origin, $weight, $height, $width, $length){

		// Minimo de 300g
		$weight = ($weight < 0.300) ? 0.300 : $weight;

		// Peso maximo de 10kg
		$weight = ($weight > 10) ? 10 : $weight;

		$height = ceil((float)$height);
		$width 	= ceil((float)$width);
		$length = ceil((float)$length);

		// Debug Logic
		// echo '<pre><br>Dados de envio:<br>';
		// echo 'zip_destination:'.$zip_destination.'<br>';
		// echo 'zip_origin:'.$zip_origin.'<br>';
		...
		$data['destinyCep'] = $zip_destination;
		$data['height'] = $height;
		$data['length'] = $length;
		$data['originCep'] = $zip_origin;
		$data['weight'] = $weight;
		$data['width'] = $width;

		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://pagseguro.uol.com.br/para_seu_negocio/howItWorksCalculator.jhtml');
		...
		$output = curl_exec($ch);
		// Debug Logic
		// echo '<pre><br>Dados de retorno:<br>';
		// var_dump($output);
		// echo '</pre>';
		$output = json_decode($output, true);

		// Se não for possivel enviar via Envio Facil, o preco sera de 20 reais
		if($output['success'] == false) {
			return 20;
		}

		//$output['standard'] = number_format((float)str_replace(',', '.', $output['standard']), 2);
		return number_format((float)str_replace(',', '.', $output['efreteTotalValue']), 2);
	}
	...
	public function get_shipping() {
		...
		$enviofacil_key = array_search ('1', $this->services) ;
		if( $enviofacil_key ){
			$valor = $this->calcEnvioFacil(
									$this->zip_destination, 
									$this->zip_origin, 
									$package['weight'], 
									$package['height'], 
									$package['width'], 
			...
			$serv->Valor = $valor;
			$serv->PrazoEntrega = 1;
			$serv->ValorSemAdicionais = $valor;
			$serv->ValorMaoPropria = $this->own_hand;
			$serv->ValorAvisoRecebimento = $valor;
			$serv->ValorValorDeclarado = $this->declared_value;

			$serv->EntregaDomiciliar = 'S';
			$serv->EntregaSabado = 'N';

			$serv->Erro = 0;
			$serv->MsgErro = "";
  			$values[1] = $serv;
			unset($this->services[$enviofacil_key]);
		}
